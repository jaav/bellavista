...
class WP_e_Commerce_Multilingual{
    ...
    function __construct(){
        ...
        if(!is_admin()){
            if($sitepress->get_current_language() != $sitepress->get_default_language()){
                ...
                add_filter('wpsc_checkout_field_name', array($this, 'translate_checkout_form'));
            }
        ...
        add_filter('option_wpsc_shortcode_page_ids', array($this, 'translate_page_ids'));
        add_action('pre_get_posts', array($this, 'set_translated_pagename'));
    }

	function translate_page_ids($ids) {
		foreach ($ids as $string => $id) {
			$ids[$string] = icl_object_id($id, 'page');
		}
		return $ids;
	}

	function set_translated_pagename($q) {
		$prod_page = wpsc_get_the_post_id_by_shortcode('[productspage]');
		if ($q->get('page_id') == $prod_page) {
			$prod_page = get_post($prod_page);
			$q->set('page_id', '');
			$q->set('pagename', $prod_page->post_name);
		}
	}
    ...
    function filter_pre_get_posts($q){
        global $sitepress;
        if(isset($q->query_vars['post_type']) && $q->query_vars['post_type'] == 'wpsc-product-file'){
	...
	function translate_checkout_form( $checkout_item_name ){
		$label = icl_translate('wpsc', $checkout_item_name .'_checkout_form_label', $checkout_item_name);
